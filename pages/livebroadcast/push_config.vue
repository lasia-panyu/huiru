<template>
	<view class="wrap" style="min-height: 100vh; padding: 20px;">
		...
		<u-form :model="form" ref="uForm" label-width="150">
			...
			<!-- <u-form-item label="电脑推流地址" >
				<u-input v-model="form.push_url" placeholder="非电脑不填写!地址格式: rtmp://"  />
			</u-form-item> -->
			<!-- czq 20210512 -->
			<u-form-item label="房间密码" >
				<u-input v-model="form.push_pwd" placeholder="默认为公共直播间"  />
			</u-form-item>
			<!-- czq -->
			...
			<u-form-item label="美白级别" required>
...
<script>
	export default {
		data() {
			return {
				...
				form: {
					live_Name: '', //房间名称
					push_url: '',
					// rtmp://fuyin.fuxinbank.com/myapp/'+this.$util.account.id,
					// push_url: 'rtmp://57.8.11.45:1935/myapp/'+this.userInfo._id, // 推流地址
					push_pwd: '', //房间密码
					mode: 'FHD', // 视频模式
					autoFocus: true, // 自动对焦
					zoom: true, // 远近焦距
		...
		async onLoad(){			
			...
			this.form.push_url='rtmp://fuyin.fuxinbank.com:1935/myapp/'+this.userInfo._id
			console.log(this.form.push_url);
		},
		methods: {
			modeChange(index) {
			...
			launchPusher() {
				...
				let teller_id = this.userInfo._id
				let live_Name = this.form.live_Name
				let push_pwd = this.form.push_pwd
				...
				uni.request({
									...
									// dataType:'json',
								    data: {
										live_HomeNum: teller_id, //this.userInfo._id,
										live_Name: live_Name,
