<template>
	<view class="wrap">
		...
		<!-- 	<u-form-item :label-position="labelPosition" label-width="170" label="再次输入密码" prop="passwd1">
				<u-input :password-icon="true" :border="border" typ.e="password" v-model="forms.passwd1" placeholder="请再次输入密码"></u-input>
			</u-form-item> -->
					    ...
					    success: async function (res) {							
					        if (res.confirm) {
								...
								if(result.rspCode=="0000"){
									that.$util.accountno=that.forms.id
									...
									that.$util.toast(that.$refs,result.rspCode+":"+result.rspMsg,true,'error');
									return;
								}
