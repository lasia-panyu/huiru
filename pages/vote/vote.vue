	...
	<view class="qizai-cardList__container">
		<view class="qizai-cardList__card" v-for="item,index in videoList" :key="item.post_id"  @click="clickCard(index)">
			...
			<view  class="qizai-cardList__card-desc">
				...
				<view class="qizai-cardList__card-header" style="text-align: right;">
...
<script>
	export default {
		...
		async onShow(option){
			console.log(option)
			let videoList=await this.$util.post('newsVideoSel',{'formType':'V',teller_id:'001882'}); 
			
	              /* uni.request({
					method:'POST',
					
				    url: 'http://57.8.11.43:10066/PanguService/newsVideoSel', //数据接口。
					
					dataType:'json',
				    data: {'formType':'V',teller_id:'001882'},
				    header: {
