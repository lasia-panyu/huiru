<template>
	...
	<!-- 		<block v-for='(item,key) in forms' >
				<u-action-sheet v-if="item.type == 'select'" :list="item.value" v-model="item.tap" @click="actionSheetCallback"></u-action-sheet>
				<u-form-item :label-position="labelPosition" :label="item.label" label-width="150"  :prop="item.prop"> 
					<u-input :border="border" :type="item.type" :select-open="false" v-model="item.result" :placeholder="item.placeholder" @click="click(key,item)"></u-input>
...
<!-- 			<u-form-item :label-position="labelPosition" label="记住密码" prop="remember" label-width="150">
			...
			<u-button form-type='submit' @click="submit">提交</u-button>
		</u-form>
...
export default {
	...
	methods: {
		...
		actionSheetCallback(index) {
			uni.hideKeyboard();
			console.log(index);
			console.log(this.forms[this.currentIndex]['value'][index]);
			this.forms[this.currentIndex].result=this.forms[this.currentIndex]['value'][index]['text'];
			console.log(this.forms[this.currentIndex].result);
